package br.com.integrador.petshop.testes;

import java.util.List;

import br.com.integrador.petshop.model.Petshop;
import br.com.integrador.petshop.persistencia.PetshopDAO;

public class TestePetshop {
	public static void main(String[] args) {
		
// CADASTRAR PETSHOP
	
//		Petshop petshop = new Petshop(1, "Pet Feliz", "43.631.467/0001-40", "pet.feliz@pet.com", "thorzinho123", "051 9 8494-7994");
//
//		PetshopDAO pDAO = new PetshopDAO();
//		pDAO.cadastrar(petshop);
		
// EDITAR PETSHOP
		
//		Petshop petshop = new Petshop(1, "Pet Feliz", "43.631.467/0001-42", "petfeliz@gmail.com", "belinha123", "051 9 8494-7993");
//		
//		PetshopDAO pDAO = new PetshopDAO();
//		pDAO.editar(petshop);
		
// EXCLUIR PETSHOP
		
//		PetshopDAO pDAO = new PetshopDAO();
//		pDAO.excluir(1);
		
// BUSCAR POR ID PETSHOP
		
//		PetshopDAO pDAO = new PetshopDAO();
//		Petshop petshop = (Petshop) pDAO.buscarPorId(1);
//		System.out.println("Petshop buscada por ID: " + petshop.getNomePetshop());
		
//BUSCAR TODOS PETSHOP
		
//		PetshopDAO pDAO = new PetshopDAO();
//		List<Object> listaPetshop = pDAO.buscarTodos();
//		System.out.println("Lista de PetShops: ");
//		for(int i = 0; i < listaPetshop.size(); i++) {
//			System.out.println(((Petshop) listaPetshop.get(i)).getNomePetshop());
//		}
	}
}
